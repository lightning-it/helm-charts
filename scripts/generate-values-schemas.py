#!/usr/bin/env python3
"""Generate deterministic structural JSON schemas for product-chart values."""
...
def schema_for(value: object) -> dict[str, Any]:
    ...
    if isinstance(value, dict):
        ...
        return {
            ...
            # Wrapper charts intentionally pass arbitrary upstream values through
            # dependency aliases; known keys remain type-checked while extension
            # keys are permitted.
...
def main() -> int:
    ...
    if not charts:
        raise ValueError("no product charts found under charts/")
    for chart in charts:
